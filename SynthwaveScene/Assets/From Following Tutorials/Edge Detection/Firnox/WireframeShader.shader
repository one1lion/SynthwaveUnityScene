Shader "Lit/WireframeShader"
{
    Properties
    {
        ...
        _WireframeColor ("Wireframe color", Color) = (92,200,210,1)
    ...
    SubShader
    {
        ...
        // NB: We add the blend mode in so that we alias our wireframe.
        ...
        Pass
        {
            ...
            {
                v2f o;
                // We push the conversion to ClipPos into the geom function as
            ...
            void geom(triangle v2f IN[3], inout TriangleStream<g2f> triStream)
            {
                ...
                // We're fine using if statements since it's a trivial function.
            ...
            {
                // Calculate the unit width based on a triangle size.
                float3 unitWidth = fwidth(i.barycentric);
                // Alias the line a bit.
                float3 aliasing = smoothstep(float3(0.0, 0.0, 0.0), unitWidth * _WireframeAliasing, i.barycentric);
                // Use the coordinate closest to the edge.
                float alpha = 1 - min(aliasing.x, min(aliasing.y, aliasing.z));
                // Set to our forwards facing wireframe color.
